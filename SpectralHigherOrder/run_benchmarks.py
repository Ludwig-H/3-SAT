...
# Add the workspace root to sys.path to allow importing from SpectralHigherOrder
sys.path.append("/workspaces/3-SAT")

from SpectralHigherOrder.spectral_higher_order import (
    solve_3sat_spectral_higher_order,
    count_unsat_clauses
)
...
def run_all_benchmarks():
    ...
    markdown_lines = []
    markdown_lines.append("# Rapport de Performance: Solveur Spectral Higher-Order pour 3-SAT\n")
    markdown_lines.append("Ce tableau présente les performances du nouveau solveur spectral **Higher-Order** comparé aux baselines CDCL (Glucose4), Max-SAT exact (RC2), et WalkSAT (Random-start).\n")
    markdown_lines.append("L'analyse met en évidence le gain en temps de calcul et en nombre d'itérations (flips) obtenu grâce à l'initialisation spectrale higher-order (warm-start) par rapport à un départ aléatoire (random-start).\n")
    ...
    headers = [
        ...
        "HO Spectral Seul",
        "HO Spectral + WalkSAT (Target=0)",
        "WalkSAT Pur (Ciblant HO Unsat)",
        "Flips Gagnés (%)"
    ]
    ...
    for name, n_vars, clauses in bench_list:
        ...
        # 4. HO Spectral Seul
        ho_spins, t_ho, ho_unsat = solve_3sat_spectral_higher_order(n_vars, clauses, u=1.0, kappa=1.0)
        print(f"HO Spectral Seul: Unsat = {ho_unsat} en {t_ho:.4f}s")
        
        # 5. HO Spectral + WalkSAT
        ...
        print(f"HO Spectral + WalkSAT: Unsat = {ho_walk_unsat} en {t_total_ho_walk:.4f}s (WalkSAT: {t_ho_walk:.4f}s, {flips_ho_walk} flips)")
        
        # 6. Pure WalkSAT targeting HO Unsat
        ...
        # We compare flips of Pure WalkSAT (Target=0) vs. (HO Spectral + WalkSAT)
    ...
    report_path = "/workspaces/3-SAT/SpectralHigherOrder/benchmark_results.md"
